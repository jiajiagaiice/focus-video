...
- 一个真实运行时能力检查与播放入口：检测 ffplay/ffmpeg/nvidia-smi/trtexec，`--play` 在系统安装 ffplay 后会调用真实本地视频播放后端；`--with-tensorrt-sr` 会在缺少 TensorRT 引擎或帧桥时硬失败，避免把未实现能力伪装成可用。
- 单元测试，覆盖 3060 Ti 目标机型的默认档位、降级策略、帧级超分导出路径和运行时能力阻塞原因。
- Windows 播放器、实时超分和离线视频导出的架构文档。

## 构建与测试

本仓库默认会在发现 FFmpeg 开发库时构建进程内 demux/decode smoke path；CUDA、NVDEC、TensorRT 和 D3D 渲染后端仍未作为可用后端编译进来。每次改动都需要同步更新 [`CHANGELOG.md`](CHANGELOG.md)。
...
```bash
./build/focus-video --check-runtime --play input.mp4 --with-tensorrt-sr --trt-engine model.engine
```

安装 Ubuntu 本地播放 smoke test 依赖并验证 `ffplay` 后端真的能启动：

```bash
sudo scripts/install_runtime_deps_ubuntu.sh
scripts/smoke_local_playback.sh
```

运行进程内 FFmpeg 探测/解码 smoke path（需要 FFmpeg 开发库）：

```bash
sudo apt-get install -y libavformat-dev libavcodec-dev libavutil-dev libswscale-dev
scripts/smoke_ffmpeg_decode.sh
./build/focus-video --inspect-video input.mp4
./build/focus-video --decode-smoke input.mp4 --max-frames 8
./build/focus-video --backend-status
```

运行本地视频播放入口（仍可用于快速人工播放验证，需要系统安装 FFmpeg 的 `ffplay`）：

```bash
./build/focus-video --play input.mp4
```

> 当前 `--inspect-video` / `--decode-smoke` 已经走进程内 FFmpeg demux/decode；`--play` 仍调用真实 `ffplay` 后端做人工播放验证。D3D11 渲染、NVDEC GPU frame pool 和 TensorRT 推理后端目前会通过 `--backend-status` 明确报告未编译/未实现，而不是静默退化成假超分。当前通用容器无法补出 NVIDIA RTX GPU、驱动和 TensorRT engine；这部分必须在目标 NVIDIA 机器上验证。
...
在 Ubuntu/Linux CI 中交叉编译可分发的 Windows x64 ZIP 包：
...
生成的包会输出到 `dist/FocusVideo-0.1.0-win64.zip`，包含 `focus-video.exe`、README、LICENSE 和 Windows 使用说明。当前交叉编译包未捆绑 Windows FFmpeg 开发库，因此包内进程内 FFmpeg 路径可能不可用；本地播放仍要求 Windows 系统中安装 FFmpeg 并让 `ffplay.exe` 位于 `PATH`。
